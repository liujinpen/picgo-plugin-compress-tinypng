...
async function tinypngKeyCompress(ctx, imageUrl, key) {
  ...
  const fileDataPromise = imageUrl.startsWith("http") ? utils.fetchImage(ctx, imageUrl) : fs.readFile(imageUrl);

  return await fileDataPromise.then(fileData => {
    ...
    return ctx.request(fetchOptions).then(response => {
      ...
      if (response.status && response.status >= 200 && response.status <= 299) {
        ...
        return utils.fetchImage(ctx, location).then(res => {
          ...
          return {
            input: location, // 地址变成压缩后的地址了.
            output: utils.getImageInfo(imageUrl, res.data)
          }
        })
      }
